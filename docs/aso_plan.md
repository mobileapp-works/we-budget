...
> **そのままコピペできる形**で書いてある。文字数は App Store の上限に収めて計算済み。
...
### Subtitle（30字以内）— 29字
```
Split expenses, settle in a tap
```

### Promotional Text（170字以内）
```
Snap a receipt and log it in seconds. WeBudget tracks who paid what, calculates the balance between you two automatically, and settles up in one tap. Bills on autopilot, budget alerts before you overspend.
```
