...
| Apple / Google ログイン（ネイティブ signInWithIdToken） | クライアント完了 | dev build必須。残りはユーザーのGCP/Apple設定＋Supabaseプロバイダ有効化（SUPABASE_SETUP §7.1） |
...
- 検証: typecheck / jest 75件 / `expo export --platform ios`（バンドル生成成功＝新依存の Metro 解決OK）すべてパス。**残はユーザー作業**（GCP/Apple 設定＋dev build。SUPABASE_SETUP §7.1）。
...
- [~] **Apple / Google ログイン**: **クライアント実装完了（2026-07-05・ネイティブ `signInWithIdToken` 方式）**。残りはユーザーの外部設定＋dev build。
  ...
  - **残（ユーザー作業）**: SUPABASE_SETUP **§7.1** の手順 — ①Apple capability＋Supabase Apple 有効化 ②GCPで iOS/Web OAuthクライアント作成 ③app.json の `iosUrlScheme` を reversed iOS client ID に置換 ④.env に web/ios client id ⑤Supabase Google 有効化＋Authorized Client IDs ⑥dev build 再作成して実機確認。
...
- Apple / Google ログインはネイティブ方式のため **dev build でのみ実動作**（Expo Go/モックでは押すとデモユーザーでログイン）。実利用には GCP/Apple 設定＋Supabaseプロバイダ有効化が必要（SUPABASE_SETUP §7.1）。app.json の google-signin `iosUrlScheme` は**プレースホルダ**のまま＝ユーザーが置換するまで実 Google ログインは失敗する。
