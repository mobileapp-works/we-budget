...
> 最終更新: 2026-07-05。スレッド/モデルを切り替えながら開発するため、**このファイルが現状の正**。着手前にここを読むこと。
...
- バックエンド: Supabase（**構築済み・実接続で動作中**。`EXPO_PUBLIC_USE_MOCK=false`）
  ...
  - マイグレーション 0001〜0004 適用済み（テーブル/RLS/RPC/Storage/精算・ペア解除バグ修正）
...
supabase/
  migrations/             # 0001_init / 0002_rls_functions / 0003_storage / 0004_fix_settlement_leave_pair（適用済み）
...
### 🔴 審査ブロッカー（リリース必須・軽い順）
- [ ] **プライバシーポリシー公開**: コード側は準備完了（アプリのリンク先 `APP_CONFIG.privacyPolicyUrl` は既に公開予定URLを指す）。**残りは GitHub Pages を ON にするだけ**（ダッシュボード操作＝ユーザー作業）。
  ...
  - **残（ユーザー作業）**: GitHub → repo `mobileapp-works/we-budget` の Settings → Pages → Source=「Deploy from a branch」→ Branch=`main` / フォルダ=`/docs` → Save。数分後に公開。
  - **公開URL**: `https://mobileapp-works.github.io/we-budget/privacy-policy.html`（`git remote` の owner=mobileapp-works で確定。アプリ constants の値と一致）。
  - **確認**: `curl -s -o /dev/null -w "%{http_code}" <URL>` が `200` を返せば公開成功（2026-07-05 時点は未ONで 404）。公開後、App Store Connect の「Privacy Policy URL」にも同URLを設定。
- [ ] **delete-account Edge Function デプロイ**: 関数コード（`supabase/functions/delete-account/index.ts`）とアプリ側呼び出し（`supabaseBackend.deleteAccount` → `sb.functions.invoke('delete-account')`、本人セッションJWT付き）は準備完了・整合確認済み。**残りはデプロイのみ**（ダッシュボード操作＝ユーザー作業）。
  - **残（ユーザー作業）**: Supabaseダッシュボード → Edge Functions → Deploy a new function → Via Editor → 関数名 `delete-account` → 上記ファイルの中身を丸ごと貼り付け → Deploy。**Verify JWT は ON（既定）のまま**（アプリが本人JWTを付けて呼ぶため。push関数のOFFとは逆なので混同注意）。Secret `SUPABASE_SERVICE_ROLE_KEY` は自動注入で追加設定不要。
  - **確認（配線）**: `curl -s -o /dev/null -w "%{http_code}" -X POST https://oarbchcwtfxjxvjnuoaw.supabase.co/functions/v1/delete-account` が **未デプロイ=404 / デプロイ済(JWTなし)=401**（2026-07-05 時点は 404）。
  - **確認（本番動作）**: ⚠️実行するとアカウントが本当に消える。**使い捨てアカウント**でログイン → 設定 → アカウントを削除 → 実行 → 成功してログアウトされるか確認（本番アカウントでやらない）。
...
- [x] ログアウト（設定画面）
- [ ] アカウント削除（UIあり・呼び出し整合確認済み。**delete-account デプロイまで動作しない**。関数エンドポイントは 2026-07-05 時点 404＝未デプロイ）
- [ ] プライバシーポリシーの公開URL（html準備済み・**2026-07-05 に OCR記述を端末内OCRへ修正**、アプリのリンク先も設定済み。**GitHub Pages 未ON＝URLは 404**）
