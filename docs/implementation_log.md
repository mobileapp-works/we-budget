...
- [~] **プッシュ通知の実配信**: **クライアント実装完了**。残りはユーザー側のデプロイ＋Webhook設定＋push認証情報のみ。
  ...
  - **残（ユーザー作業）**: ①`supabase functions deploy send-push-notification`（＋任意 `supabase secrets set PUSH_WEBHOOK_SECRET=xxx`）→ ②ダッシュボードで **Database Webhook** 作成（Database > Webhooks / table=`notifications` / event=INSERT → HTTP Request で関数URL・header `x-webhook-secret` を付与）→ ③**push認証情報**: iOSは `eas build` 時にAPNsキー自動生成、AndroidはExpo/EASにFCM(V1)サービスアカウントを登録。→ ④dev build を実機で起動して許可＆受信確認。
